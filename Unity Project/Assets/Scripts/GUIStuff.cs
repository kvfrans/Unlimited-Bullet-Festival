...
public class GUIStuff : MonoBehaviour {
	...
	void OnGUI()
	{
        ...
        greenStyle.normal.background = MakeTex( 2, 2, new Color( 0f, 1f, 0f, greenness ) );
		GUI.Box (new Rect (0, 0, Screen.width, Screen.height), "", greenStyle);

		redStyle = new GUIStyle( GUI.skin.box );
        redStyle.normal.background = MakeTex( 2, 2, new Color( 1f, 0f, 0f, redness ) );
		GUI.Box (new Rect (0, 0, Screen.width, Screen.height), "", redStyle);
		...
		if(hero == "meanie" && cuttext != "")
		{
			GUI.DrawTexture(new Rect(10, 10, 384, 448), Resources.Load<Texture2D>("guy2"), ScaleMode.StretchToFill, true, 10.0F);
		}
	}
